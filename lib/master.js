...
async function http_get_json(url, fetch_mode=17) {
	/*
	 * @inspire: {
	...
	*/
	...
	if ( (fetch_mode & 1)===2 ) { opt.method = 'POST'; }
	if ( (fetch_mode & 1)===4 ) { opt.method = 'HEAD'; }
	if ( (fetch_mode & 1)===8 ) { opt.method = 'CONNECT'; }
	const xhr = await fetch(url);
	if ( ! await xhr.ok ) { throw {errortext: 'HTTP Error', ok: await xhr.ok, message: await xhr.statusText, code: await xhr.status, redirected: await xhr.redirected } }
